...
#
# Real-training SFT launcher for Quartz hopper / BR200 gpu.
#
# Usage:
#   sbatch -p hopper --qos=hopper scripts/run_sft_quartz.sh \
...
#       model.phase2_adapter_path=/.../hcl_substage2/final_model \
#       run.run_name=sft_align_real
...
if [[ -f "${CONDA_BASE}/etc/profile.d/conda.sh" ]]; then
  ...
  echo "[sft] FATAL: ${CONDA_BASE}/etc/profile.d/conda.sh not found" >&2; exit 64
fi
PY="$(command -v python)"
[[ "${PY}" == "${CONDA_ENV_PATH}/bin/python" ]] || {
  echo "[sft] FATAL: wrong python=${PY}" >&2; exit 64
}
...
echo "[sft] host=$(hostname) partition=${SLURM_JOB_PARTITION:-?} job=${SLURM_JOB_ID:-?}"
...
if [[ "${NUM_GPUS}" -gt 1 ]]; then
  exec torchrun --standalone --nproc_per_node="${NUM_GPUS}" \
      -m stages.downstream.sft.train \
      --config "${CONFIG}" "$@"
else
  exec python -u -m stages.downstream.sft.train \
      --config "${CONFIG}" "$@"
fi
